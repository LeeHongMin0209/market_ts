...
import { Get, HttpException, HttpStatus, Inject, Injectable, NotFoundException, Param, Post } from '@nestjs/common';
...
export class MarketService {
  ...
  async getBestMarketList(page: string, type: string): Promise<Market[]> {
  ...
  async getMarket(marketId: string): Promise<Market> {
    const market: MarketDocument | null = await this.marketModel.findById(marketId).select('-_id image name url introduce like allowStatus tags');
